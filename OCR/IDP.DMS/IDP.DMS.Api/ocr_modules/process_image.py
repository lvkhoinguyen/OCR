...
def padding_image(image, width, height):
    ...
    if h < height and w < width:
        # Ảnh nhỏ hơn cả hai chiều → padding bốn phía
        top    = int((height - h) / 2)
        ...
        # Ảnh lớn hơn cả hai chiều → ưu tiên scale theo chiều cao
        left, right = 0, int((width * h) / height)
        new_img = cv2.copyMakeBorder(
            image, 0, 0, left, right, cv2.BORDER_CONSTANT, value=color)
    elif h > height:
        # Chỉ chiều cao vượt → padding chiều rộng theo tỉ lệ
        left, right = 0, int((width * h) / height)
        new_img = cv2.copyMakeBorder(
            image, 0, 0, left, right, cv2.BORDER_CONSTANT, value=color)
    elif w > width:
        # Chỉ chiều rộng vượt → padding chiều cao theo tỉ lệ
        top = int(((height * w) / width) / 2)
        bottom = top
        new_img = cv2.copyMakeBorder(
            image, top, bottom, 0, 0, cv2.BORDER_CONSTANT, value=color)
    else:
        # h == height hoặc w == width → không cần padding
        new_img = image
